...
fn main() -> ! {
    ...
    ufmt::uwriteln!(&mut serial_hw, "Hello from Arduino Leonardo!").unwrap_infallible();
    ...
    let string_descriptors = StringDescriptors::new(LangID::EN_US)
        ...
        .product("Arduino Leonardo (avr-hal)")
        .serial_number("leonardo-usb-serial-example");

    let vid_pid = UsbVidPid(0x2341, 0x8036);

    let mut usb_dev = UsbDeviceBuilder::new(&usb_bus_allocator, vid_pid)
