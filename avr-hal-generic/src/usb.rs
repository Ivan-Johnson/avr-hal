...
#[derive(Default)]
pub struct EndpointTableEntry {
    // REVIEW: what should the scoping be here?
...
pub macro create_usb_bus(
    ...
    impl<S: $SuspendNotifier> $AvrGenericUsbBus<S> {
        ...
        pub fn set_current_endpoint(
            ...
            unsafe {
                // TODO: investigate unsafety here (only occurs in atmega8u2?)
        ...
        pub fn endpoint_byte_count(&self, cs: CriticalSection) -> u16 {
            // REVIEW: should this conditionally be a u8
            let usb = self.usb.borrow(cs);
