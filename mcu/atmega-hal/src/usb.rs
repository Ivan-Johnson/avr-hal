...
impl<S: SuspendNotifier> UsbDeviceBus for UsbdBus<S> {
    ...
    fn enable(&mut self) {
        interrupt::free(|cs| {
            ...
            usb.udien().modify(|_, w| w.eorste().set_bit().sofe().set_bit());
        });
    }
    ...
    fn reset(&self) {
        interrupt::free(|cs| {
            ...
            usb.udint().clear_interrupts(|w| w.wakeupi().clear_bit().suspi().clear_bit());
            usb.udien().modify(|_, w| w.wakeupe().clear_bit().suspe().set_bit());
        })
    }
    ...
    fn write(&self, ep_addr: EndpointAddress, buf: &[u8]) -> Result<usize, UsbError> {
        interrupt::free(|cs| {
            ...
            if endpoint.eptype_bits == EP_TYPE_CONTROL {
                ...
                usb.ueintx().clear_interrupts(|w| w.txini().clear_bit().rxouti().clear_bit());
                ...
                //NB: RXOUTI serves as KILLBK for IN endpoints and needs to stay zero:
                usb.ueintx().clear_interrupts(|w| w.fifocon().clear_bit().rxouti().clear_bit());
            }
    ...
    fn read(&self, ep_addr: EndpointAddress, buf: &mut [u8]) -> Result<usize, UsbError> {
        interrupt::free(|cs| {
            ...
            if endpoint.eptype_bits == EP_TYPE_CONTROL {
                ...
                usb.ueintx().clear_interrupts(|w| w.rxouti().clear_bit().rxstpi().clear_bit());
    ...
    fn set_stalled(&self, ep_addr: EndpointAddress, stalled: bool) {
        interrupt::free(|cs| {
            ...
            if self.set_current_endpoint(cs, ep_addr.index()).is_ok() {
                usb.ueconx().modify(|_, w| w.stallrq().bit(stalled).stallrqc().bit(!stalled));
            }
        });
    }
    ...
    fn suspend(&self) {
        interrupt::free(|cs| {
            let usb = self.usb.borrow(cs);
            usb.udint().clear_interrupts(|w| w.suspi().clear_bit().wakeupi().clear_bit());
            usb.udien().modify(|_, w| w.wakeupe().set_bit().suspe().clear_bit());
